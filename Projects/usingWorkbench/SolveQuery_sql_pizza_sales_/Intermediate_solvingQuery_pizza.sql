...
select pizza_types.category, sum(orders_details.quantity) as quant
...
select round(avg(quant),0) as avg_pizza_ordered_per_day from 
(select orders.order_date, sum(orders_details.quantity) as quant
